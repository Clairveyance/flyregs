...
import { AdComplianceModal, DatePickerModal } from '@/components/AdComplianceModal'
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          {aircraft.image_path ? (
            <Pressable onPress={canEdit ? handlePickAircraftImage : undefined} disabled={photoBusy}>
              {(() => {
                const heroUrl = getAircraftImageUrl(aircraft.image_path)
                return heroUrl ? <RetryImage uri={heroUrl} style={styles.heroImage} /> : null
              })()}
              {photoBusy && (
            ...
            try {
              ...
              if (existing) {
                ...
                if (!nextDueDate && targetId) {
                  // Checked, not bare: supabase-js resolves {data, error}
                  // rather than throwing, so the enclosing try could never
                  // catch this. Silently failing here re-arms the false
                  // "due in 14 days" push for an hours-based AD a year out,
                  // which is the exact thing this write exists to prevent.
      ...
      <EditAircraftModal
        ...
        onSaved={(identityChanged) => {
          ...
          resyncAircraftAds(aircraft.id)
            // console.error is invisible on a real device. If this fails after
            // an identity edit, the Applicable ADs list silently keeps the OLD
            // aircraft's ADs -- a 172S edited to a PA-28-181 keeps all 13
            // Cessna ADs -- while still presenting them as "matched to what
            // you actually fly". Surfaced the same way handleBackfillAds does
            // for the identical call, so the user knows to tap Recheck.
            .catch((e: any) => confirm({
              title: 'Could not update Applicable ADs',
              message: `${e?.message ?? 'Unknown error'}\n\nThis aircraft may still be showing the previous model's ADs. Tap Recheck to try again.`,
              cancelLabel: null,
            }))
