...
export default function PcgTermScreen() {
  ...
  useEffect(() => {
    if (!term) return
    loadHighlightSets(term.slug, 'pcg').then(({ texts, shared }) => { setHighlightedBlockTexts(texts); setSharedHighlights(shared) })
  }, [term])
  ...
  const handleToggleHighlight = useCallback(async (paraText: string) => {
    ...
    try {
      ...
      const { texts, shared } = await loadHighlightSets(term.slug, 'pcg')
      setSharedHighlights(shared)
      setHighlightedBlockTexts(texts)
    } finally {
      toggleInFlight.current = false
    }
  }, [term, hasPlusAccess, authLoading])
                    ...
                    linkifyText(para).map((seg, si) =>
                      seg.route ? (
                        <Text
                          key={si}
                          onPress={() => {
                            if (!hasProAccess) { if (!authLoading) router.push('/paywall?tier=pro' as any); return }
