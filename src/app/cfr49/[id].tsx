...
export default function Cfr49SectionScreen() {
  ...
  // Told apart from "section is null", which this screen used to render as
  ...
  useEffect(() => {
    if (!id) return
    loadHighlightSets(id, 'cfr49').then(({ texts, shared }) => { setHighlightedBlockTexts(texts); setSharedHighlights(shared) })
  }, [id])
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setLoadFailed(!!secRes.error && secRes.error.code !== 'PGRST116')
      setLoading(false)
    })
  ...
  const handleToggleHighlight = useCallback(async (paraText: string) => {
    ...
    try {
      ...
      const { texts, shared } = await loadHighlightSets(section.section_number, 'cfr49')
