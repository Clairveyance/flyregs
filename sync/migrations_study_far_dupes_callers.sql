-- Regenerated mechanically from live definitions; all three surfaces
-- (queue / pool count / mastery) change TOGETHER so they can't disagree.
-- See migrations_study_far_dupes.sql.
...
AS $function$
  ...
  fresh_far AS (
    ...
    WHERE (p_item_types IS NULL OR 'far' = ANY(p_item_types))
      ...
      -- Within-part-unique titles only (study_far_sections): the card asks
      -- "Which Part N rule covers X?", so a title repeated inside its own
      -- part is unanswerable. See migrations_study_far_dupes.sql.
...
CREATE OR REPLACE FUNCTION public.get_study_mastery(p_item_type text DEFAULT NULL::text)
...
AS $function$
  ...
  SELECT
    (SELECT count(*) FILTER (WHERE correct_streak >= 3) FROM study_progress WHERE user_id = auth.uid() AND (p_item_type IS NULL OR item_type = p_item_type))::int AS mastered,
    (SELECT count(*) FROM study_progress WHERE user_id = auth.uid() AND (p_item_type IS NULL OR item_type = p_item_type))::int AS seen,
    avail.total::int AS total_available,
    CASE WHEN avail.total = 0 THEN 0
      ELSE round((SELECT count(*) FILTER (WHERE correct_streak >= 3) FROM study_progress WHERE user_id = auth.uid() AND (p_item_type IS NULL OR item_type = p_item_type)) * 100.0 / avail.total)::int
    END AS pct
  FROM avail;
$function$;
