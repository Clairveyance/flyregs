#!/usr/bin/env python3
"""
Aviation Mechanic ACS Scraper
=============================================================================
FAA-S-ACS-1 (Aviation Mechanic General, Airframe, and Powerplant ACS) --
the third and last of the 3 documents acs_scraper.py's own docstring
flagged as "structurally different" and deferred (it doesn't even use the
phrase "Area of Operation" anywhere, confirmed by direct search -- that
alone was enough to make the standard scraper return 0/0/0).

Real structure, confirmed against the actual PDF: same underlying
AM.<area>.<subject>.<K|R|S><n> element-code taxonomy as every other ACS
document, genuinely different labels throughout:
  - Only 3 top-level Areas total, no restart: "I. General", "II. Airframe",
    "III. Powerplant" -- these are the 3 exam sections combined into one
    PDF, but unlike the multi-section PTS documents they do NOT repeat
    Area numbering per section (each area is used exactly once), so this
    loads as ONE RefPack with 3 areas, not 3 separate packs.
  - The task-equivalent unit is literally the word "Subject", not "Task"
    ("Subject A. Fundamentals of Electricity and Electronics").
  - No "References" field at all for any subject (confirmed: 0 occurrences
    in the whole document).
  - "Objective"/"Knowledge"/"Risk Management"/"Skills" labels have no
    colon, unlike the standard ACS convention.
  - Real Knowledge, Risk Management, AND Skills content throughout (not a
    knowledge-only document like the UAS/Military-Competency ACS docs) --
    confirmed 40 real subjects, 1195 elements total, matching an
    independent raw element-code count exactly.

Modes:
  test    parse the already-downloaded PDF, no DB writes, prints counts
  full    fetches the doc, parses, upserts

Usage:
  python aviation_mechanic_acs_scraper.py --mode test --pdf /path/to.pdf
  python aviation_mechanic_acs_scraper.py --mode full

Environment variables required for full mode:
  SUPABASE_URL             e.g. https://abcdefg.supabase.co
  SUPABASE_SERVICE_KEY     service_role secret key (not anon key)
"""

import argparse
import logging
import os
import re
import sys

import requests
import fitz

logging.basicConfig(level=logging.INFO, format="%(message)s")
log = logging.getLogger("aviation_mechanic_acs_scraper")

SUPABASE_URL = os.environ.get("SUPABASE_URL", "").rstrip("/")
SUPABASE_KEY = os.environ.get("SUPABASE_SERVICE_KEY", "")

DOC = {
    "code": "FAA-S-ACS-1",
    "title": "Aviation Mechanic General, Airframe, and Powerplant Airman Certification Standards",
    "slug": "aviation-mechanic-general-airframe-powerplant",
    "doc_type": "acs",
    "pdf_url": "https://www.faa.gov/training_testing/testing/acs/Aviation_Mechanic_Certification_Standards.pdf",
}

AREA_RE = re.compile(r"\n([IVXLC]+)\.\s+([A-Za-z][A-Za-z0-9 ,'/&()-]*?)\n")
TASK_RE = re.compile(
    r"Subject ([A-Z])\.\s+([^\n]+)\nObjective\s*\n(.+?)\nKnowledge\s*\n(.+?)"
    r"(?=\nSubject [A-Z]\.|\n[IVXLC]+\.\s+[A-Za-z]|\Z)",
    re.DOTALL,
)
ELEMENT_RE = re.compile(
    r"\b(AM\.[IVXLC]+\.[A-Z]\.[KRS]\d+[a-z]?)\s+(.+?)"
    r"(?=\nAM\.[IVXLC]+\.[A-Z]\.[KRS]\d+[a-z]?\s|\Z)",
    re.DOTALL,
)
TYPE_MAP = {"K": "knowledge", "R": "risk_management", "S": "skill"}


def extract_pdf_text(path: str) -> str:
    doc = fitz.open(path)
    try:
        text = "\n".join(page.get_text() for page in doc)
    finally:
        doc.close()
    return text.replace("\x00", "")


def parse_document(full_text: str) -> dict:
    area_matches = list(AREA_RE.finditer(full_text))
    task_matches = list(TASK_RE.finditer(full_text))

    areas_seen: dict = {}
    tasks = []
    elements = []
    for i, tm in enumerate(task_matches):
        letter = tm.group(1)
        title = " ".join(tm.group(2).split())
        objective = " ".join(tm.group(3).split())
        knowledge_blob = "Knowledge\n" + tm.group(4)

        pos = tm.start()
        area_num, area_title = None, None
        for am in area_matches:
            if am.start() >= pos:
                break
            area_num, area_title = am.group(1), am.group(2).strip()
        if area_num is not None and area_num not in areas_seen:
            areas_seen[area_num] = area_title

        tasks.append({
            "area_number": area_num, "task_letter": letter, "title": title,
            "objective": objective, "references_text": None, "sort_order": i,
        })

        for code, body in ELEMENT_RE.findall(knowledge_blob):
            clean_body = " ".join(body.split())
            # Only the document's very last element (AM.III.M.S12, its
            # final Skill) has no following element or Subject/Area
            # heading to stop at, and sweeps up the trailing "Appendix 1:
            # Practical Test Roles..." section along with a bled-in page
            # number -- confirmed the ONLY outlier across all 1195
            # elements. Cut at "Appendix" if present, then strip a
            # trailing bare page number either way.
            clean_body = re.split(r"\s+Appendix\s", clean_body)[0]
            clean_body = re.sub(r"\s+\d{1,3}$", "", clean_body)
            kind = TYPE_MAP.get(code.split(".")[-1][0], "unknown")
            elements.append({
                "area_number": area_num, "task_letter": letter, "element_code": code,
                "element_type": kind, "body_text": clean_body,
            })

    areas = [{"area_number": n, "title": t, "sort_order": i} for i, (n, t) in enumerate(areas_seen.items())]
    return {"areas": areas, "tasks": tasks, "elements": elements}


def _headers(extra=None) -> dict:
    h = {"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}", "Content-Type": "application/json"}
    if extra:
        h.update(extra)
    return h


def upsert_document(doc: dict) -> None:
    resp = requests.post(
        f"{SUPABASE_URL}/rest/v1/acs_documents",
        headers=_headers({"Prefer": "resolution=merge-duplicates,return=minimal"}),
        params={"on_conflict": "code"},
        json=[{"code": doc["code"], "title": doc["title"], "slug": doc["slug"], "doc_type": doc["doc_type"], "pdf_url": doc["pdf_url"]}],
        timeout=30,
    )
    resp.raise_for_status()


def upsert_areas(doc_code: str, areas: list) -> None:
    if not areas:
        return
    rows = [{"doc_code": doc_code, **a} for a in areas]
    resp = requests.post(
        f"{SUPABASE_URL}/rest/v1/acs_areas_of_operation",
        headers=_headers({"Prefer": "resolution=merge-duplicates,return=minimal"}),
        params={"on_conflict": "doc_code,area_number"},
        json=rows,
        timeout=30,
    )
    resp.raise_for_status()


def upsert_tasks(doc_code: str, tasks: list) -> dict:
    rows = [{"doc_code": doc_code, **t} for t in tasks]
    resp = requests.post(
        f"{SUPABASE_URL}/rest/v1/acs_tasks",
        headers=_headers({"Prefer": "resolution=merge-duplicates,return=representation"}),
        params={"on_conflict": "doc_code,area_number,task_letter"},
        json=rows,
        timeout=30,
    )
    resp.raise_for_status()
    return {(r["area_number"], r["task_letter"]): r["id"] for r in resp.json()}


def upsert_elements(doc_code: str, elements: list, task_id_map: dict) -> None:
    rows = []
    seen = set()
    for e in elements:
        task_id = task_id_map.get((e["area_number"], e["task_letter"]))
        if not task_id:
            continue
        if e["element_code"] in seen:
            log.warning(f"  duplicate element_code {e['element_code']} -- keeping first occurrence")
            continue
        seen.add(e["element_code"])
        rows.append({
            "doc_code": doc_code, "task_id": task_id, "element_code": e["element_code"],
            "element_type": e["element_type"], "body_text": e["body_text"], "sort_order": len(rows),
        })
    if not rows:
        return
    # 1195 rows in one request risks a payload/row limit -- chunk defensively.
    chunk = 500
    for start in range(0, len(rows), chunk):
        resp = requests.post(
            f"{SUPABASE_URL}/rest/v1/acs_elements",
            headers=_headers({"Prefer": "resolution=merge-duplicates,return=minimal"}),
            params={"on_conflict": "doc_code,element_code"},
            json=rows[start:start + chunk],
            timeout=60,
        )
        resp.raise_for_status()


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--mode", choices=["test", "full"], default="test")
    ap.add_argument("--pdf", help="Local PDF path (test mode)")
    args = ap.parse_args()

    if args.mode == "test":
        if not args.pdf:
            log.error("--pdf required in test mode")
            sys.exit(1)
        parsed = parse_document(extract_pdf_text(args.pdf))
        log.info(f"Areas: {len(parsed['areas'])}")
        log.info(f"Tasks: {len(parsed['tasks'])}")
        log.info(f"Elements: {len(parsed['elements'])}")
        for a in parsed["areas"]:
            log.info(f"  {a['area_number']}: {a['title']}")
        long_ones = [e for e in parsed["elements"] if len(e["body_text"]) > 400]
        if long_ones:
            log.warning(f"  {len(long_ones)} element(s) over 400 chars -- review before trusting:")
            for e in long_ones:
                log.warning(f"    {e['element_code']}: {e['body_text'][:80]}...")
        return

    if not SUPABASE_URL or not SUPABASE_KEY:
        log.error("SUPABASE_URL / SUPABASE_SERVICE_KEY not set")
        sys.exit(1)

    resp = requests.get(DOC["pdf_url"], headers={"User-Agent": "Mozilla/5.0"}, timeout=60)
    resp.raise_for_status()
    tmp_path = f"/tmp/{DOC['slug']}.pdf"
    with open(tmp_path, "wb") as f:
        f.write(resp.content)

    parsed = parse_document(extract_pdf_text(tmp_path))
    log.info(f"parsed: {len(parsed['areas'])} areas, {len(parsed['tasks'])} tasks, {len(parsed['elements'])} elements")

    upsert_document(DOC)
    upsert_areas(DOC["code"], parsed["areas"])
    task_id_map = upsert_tasks(DOC["code"], parsed["tasks"])
    upsert_elements(DOC["code"], parsed["elements"], task_id_map)
    log.info("loaded.")


if __name__ == "__main__":
    main()
