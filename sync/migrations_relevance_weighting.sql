-- Relevance/commonality weighting, built on acs_citation_density
-- (migrations_acs_citation_density.sql). This is the signal RC asked for
-- to fix the "obscure questions" complaint: previously EVERY selection
-- query was ORDER BY random() with zero notion of "how commonly is this
-- actually tested" (see flyregs_flashcard_quiz_quality_findings.md,
-- Finding 3). far_relevance_weight/ac_relevance_weight return how many
-- distinct ACS/PTS tasks cite a given FAR part / AC number -- real FAA
-- ground truth, not a guess.
--
-- Weighting scheme: ORDER BY (weight + 1) * random() DESC. This biases
-- selection toward higher-weight items in expectation while keeping real
-- randomness (a weight-0 item can still win sometimes) -- not a
-- deterministic top-N, which would make every session look the same.
-- AIM/PCG are untouched here: AIM citation matching against
-- acs_citation_density needs paragraph-level granularity ACS references
-- don't provide (ACS only ever says bare "AIM"), and PCG has no ACS
-- citation type at all -- both stay on pure random() for now, same as
-- before.

CREATE OR REPLACE FUNCTION public.far_relevance_weight(p_part text)
 RETURNS integer
 LANGUAGE sql
 STABLE
AS $function$
  SELECT coalesce((SELECT task_count FROM acs_citation_density WHERE cited_type = 'far' AND cited_id = p_part), 0);
$function$;

CREATE OR REPLACE FUNCTION public.ac_relevance_weight(p_document_number text)
 RETURNS integer
 LANGUAGE sql
 STABLE
AS $function$
  SELECT coalesce((SELECT task_count FROM acs_citation_density WHERE cited_type = 'ac' AND cited_id = p_document_number), 0);
$function$;

-- get_study_queue: fresh_far / fresh_ac now weight toward higher-relevance
-- content instead of pure random(). Everything else (filters, the 'due'
-- CTE, pcg/aim) is untouched.
...
-- create_challenge: the FAR/AC candidate pools weight the SAME way before
-- the final random() pick from the combined pool -- a higher-relevance
-- item is more likely to be IN the 3x/1x candidate slice, not guaranteed
-- to win, so Duels still has real variety.
