-- Ask FlyRegs (hybrid_search): add curated concept anchors as a 4th RRF signal.
-- RC bug report 2026-09-03: "what is power off glide distance" never surfaced 91.205 in AFR.
-- Root cause: concept anchors were only wired into the LEXICAL search_* RPCs (Home SmartSearch),
-- never into hybrid_search, which is what Ask FlyRegs actually calls. 91.205 sits at lexical
-- rank 16 (its 2,910-char chunk mentions the terms once; LOI over-water chunks score 3x higher)
-- and poorly on vector, so RRF never lifts it into the top 15.
-- Fix: matched_anchors evaluates the 300-row anchor table once, anchor_ranked does an indexed
-- join (content_chunks_source_idx) to chunk 0 of anchored docs, contributing a flat 0.5 --
-- below citation_ranked's 1.0 (exact ID match still wins), far above RRF's ~0.016 ceiling.
-- Built additively from the LIVE definition (migrations drift), everything else byte-identical.
...
AS $function$
  ...
  matched_anchors as (
    ...
    where public.search_anchor_matches(p_query_text, a.phrase)
  ),
  anchor_ranked as (
    select c.id
    from content_chunks c
