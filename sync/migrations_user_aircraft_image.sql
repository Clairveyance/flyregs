...
--
-- Mirrors src/lib/avatar.ts's own storage shape (a public bucket, objects
-- filed under a folder named for the owning row's id) -- an aircraft photo
-- carries no more sensitivity than a profile photo, and viewer/editor
-- collaborators on a shared aircraft already see everything else about it,
-- so there's no reason to gate this one field differently.
--
-- image_path stores the STORAGE OBJECT PATH, not a public URL -- the public
-- URL is derived client-side with getPublicUrl, so this column never goes
-- stale if the bucket's public base URL ever changes.
--
-- CORRECTION, 2026-08-31 (SQL unchanged -- comment only; this migration is
-- already applied): the object name was originally the FIXED
-- <aircraft_id>/photo.jpg, upsert on re-upload, exactly like avatar.ts.
-- That was wrong here. avatar.ts persists the whole public URL including
-- its `?t=<timestamp>` cache-buster, so its stored value changes on every
-- replacement; this column persists only the path, and a fixed path meant a
-- replaced photo produced a byte-identical URL that no HTTP cache, CDN edge,
-- or React Native <Image> could tell apart from the old one. Objects are now
-- named <aircraft_id>/photo-<sha256(bytes)[:12]>.jpg, the same content-hash
-- convention the scrapers use for figure images -- see src/lib/aircraftImage.
-- ts. The policies below are unaffected: they key on
-- (storage.foldername(name))[1], which is still the aircraft id.
...
-- INSERT/UPDATE/DELETE: the object always lives in a folder named for the
-- aircraft ((storage.foldername(name))[1] is that aircraft's id) -- gate on the SAME
...
--  3. As the OWNER of a real aircraft: upload to <that aircraft's id>/
--     photo-<any hash>.jpg -- must succeed.
