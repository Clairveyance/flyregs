-- Prefer hand-authored questions when one exists for the drawn item (2026-09-01)
--
-- Duels pick a random live fact for each item. With 81 authored rows against
-- 35,275 generated ones, plain random() means a user would essentially never
-- see the authored bank, which would make authoring it pointless. This orders
-- authored first and leaves random() as the tiebreak, so an item WITHOUT an
-- authored question behaves exactly as before.
--
-- Only the fact pick is touched. The 12 other `order by random()` calls in this
-- function choose ITEMS from each corpus and are deliberately left alone.
...
begin
  ...
  loop
    ...
    if v_item.item_type in ('far', 'aim', 'ac', 'dictionary', 'cfr49') then
      ...
      -- Hand-authored questions first. They carry a study category, a
      -- recall/scenario type and a teaching explanation that the generated
      -- bank has no equivalent for; under plain random() a small authored
      -- batch would almost never be drawn against 35,000+ generated rows.
      order by (sf.origin = 'authored') desc, random()
      limit 1;
      v_have_fact := found;
    end if;
