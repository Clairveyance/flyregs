-- Wires 'dictionary' into study_facts + create_challenge's fact-first lookup.
--
-- Two real gaps found while scoping the A/D authoring pass (2026-08-16):
-- 1. study_facts_item_type_check only allows far/aim/ac/pcg -- inserting a
--    dictionary row would fail outright.
-- 2. create_challenge's `if v_item.item_type in ('far', 'aim', 'ac') then`
--    branch (the one that checks study_facts before falling back to
--    term-matching) never included 'dictionary' OR 'pcg' -- so even after
--    dictionary facts exist, Duels would keep using the term-matching
--    fallback forever unless this branch is extended. pcg is deliberately
--    LEFT AS-IS here (it has never had study_facts coverage and extending
--    it is out of scope for this pass -- don't silently expand pcg's
--    behavior as a side effect of a dictionary-only migration).

alter table public.study_facts drop constraint study_facts_item_type_check;
alter table public.study_facts add constraint study_facts_item_type_check
  check (item_type = any (array['far', 'aim', 'ac', 'pcg', 'dictionary']));
...
begin
  ...
  loop
    v_have_fact := false;
    -- Extended from ('far','aim','ac') to also check dictionary -- see
    -- migration header. pcg deliberately not added here.
